...
//     0 = not sort
//     -1 = ascending sort
export const getAllPhotos = (filters = { year: null, likes: 0, mid: null }) => {
    ActionGetPhotosRequest(filters.year);
    ...
    if (cached) {
        ActionGetPhotosSucces(AllUserPhotos);
        return;
    }
    ...
    if (!cached) {
        ...
        promise.then(
            (result) => {
                ...
                photos = selectByOneYear(photos, filters.year);
                photos = sortByLikes(photos, filters.likes);
                ActionGetPhotosSucces(photos);
            },
            (error) => {
                // вторая функция - запустится при вызове reject
                console.log("Rejected: ", error); // error - аргумент reject
                ActionGetPhotosFail(new Error(error));
            }
        );
    }
};
    ...
    VK.Api.call("photos.getAll", { owner_id: mid, extended: 1, count: count, offset: offset, v: "5.80" }, (r) => {
        try {
            ...
            if (offset <= r.response.count) {
                ...
                getMorePhotos(offset, count, mid, promise);
            } else {
